<div class="w-full h-full">
    <div class="container dashed-container flex flex-col-reverse md:flex-row md:flex">
        <div class="w-full flex md:justify-start md:mt-0 items-center justify-center mt-3 ">
            <button type="button" wire:loading.attr="disabled" wire:loading.class="!cursor-wait" onclick="$openModal('newCaseStageModal')" class="text-white bg-[#4285F4] hover:bg-[#4285F4]/90 focus:ring-4 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55">
                    ...
                    <tr>
                        <td colspan="12">
                            <div class="flex justify-center items-center text-center gap-2 py-10 w-full">
                                <x-icon name="information-circle" class="w-5 h-5" /><h1>No case stage found.</h1>
                            </div>
                        </td>
                    </tr>
                @else
                    @foreach ($caseStageList as $caseStage)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white" colspan="4">
                                {{ $caseStage->name }}
                            </th>
                            <td class="px-6 py-4" colspan="4">
                                @if ($caseStage->is_active)
                            ...
                            <td class="px-12 py-4 flex justify-end gap-4" colspan="4">
                                <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline" onclick="$openModal('editCaseStageModal')" wire:click="getSelectedCaseStageId({{ $caseStage->id }})">Edit</a>
                                <a href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline" wire:click="deleteConfirmation({{ $caseStage->id }}, '{{ $caseStage->name }}')">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                @endif
    ...
    {{-- ADD MODAL --}}
    <x-modal blur name="newCaseStageModal" align="center" max-width="md">
        <x-card title="Add New Case Stage">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="col-span-1 sm:col-span-2">
                    <div class="grid grid-cols-1 gap-5">
                        <x-input label="Case Stage Name" placeholder="Ex: Pending" class="py-3 mt-1" wire:model="name" />
            ...
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <div class="flex">
                        ...
                        <x-button primary label="Save" wire:click="addNewCaseStage" />
                    </div>
                </div>
            </x-slot>
        </x-card>
    ...
    {{-- EDIT MODAL --}}
    <x-modal blur name="editCaseStageModal" align="center" max-width="md" persistent>
        <x-card title="Edit Case Type Details" >
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="col-span-1 sm:col-span-2">
                    <div class="grid grid-cols-1 gap-5">
                        @if ($editName)
                            <x-input label="Case Stage Name" placeholder="Ex: Pending" class="py-3 mt-1" wire:model="editName" />
                        @else
                            <div class="relative w-full h-full">
                                <x-input label="Case Stage Name" placeholder="" wire:model="editName" disabled />
            ...
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <div class="flex">
                        ...
                        <x-button primary label="Save" wire:click="updateCaseStageDetails({{ $selectedCaseStageId }})" />
                    </div>
                </div>
            </x-slot>
        </x-card>
