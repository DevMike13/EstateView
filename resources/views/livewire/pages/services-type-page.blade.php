<div class="w-full h-full">
    <div class="container dashed-container flex flex-col-reverse md:flex-row md:flex">
        <div class="w-full flex md:justify-start md:mt-0 items-center justify-center mt-3 ">
            <button type="button" wire:loading.attr="disabled" wire:loading.class="!cursor-wait" onclick="$openModal('newServiceTypeModal')" class="text-white bg-[#4285F4] hover:bg-[#4285F4]/90 focus:ring-4 focus:outline-none focus:ring-[#4285F4]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#4285F4]/55">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    ...
                    @foreach ($serviceTypeList as $serviceType)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-12 py-4 flex justify-end gap-4" colspan="4">
                                <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline" onclick="$openModal('editServiceTypeModal')" wire:click="getSelectedServiceTypeId({{ $serviceType->id }})">Edit</a>
                                <a href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline" wire:click="deleteConfirmation({{ $serviceType->id }}, '{{ $serviceType->name }}')">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                @endif
        ...
        <div class="w-full flex justify-end items-end py-5 px-2">
            {{ $serviceTypeList->links() }}
        </div>
        
    </div>

    {{-- ADD MODAL --}}
    <x-modal blur name="newServiceTypeModal" align="center" max-width="md">
        <x-card title="Add New Service Type">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="col-span-1 sm:col-span-2">
                    <div class="grid grid-cols-1 gap-5">
                        <x-input label="Service Type Name" placeholder="Ex: Corporate / Business" class="py-3 mt-1" wire:model="name" />
            ...
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <div class="flex">
                        ...
                        <x-button primary label="Save" wire:click="addServiceType" />
                    </div>
                </div>
            </x-slot>
        </x-card>
    ...
    {{-- EDIT MODAL --}}
    <x-modal blur name="editServiceTypeModal" align="center" max-width="md" persistent>
        <x-card title="Edit Service Type Details" >
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="col-span-1 sm:col-span-2">
                    <div class="grid grid-cols-1 gap-5">
                        @if ($editName)
                            <x-input label="Service Type Name" placeholder="Ex: Corporate / Business" class="py-3 mt-1" wire:model="editName" />
                        @else
            ...
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <div class="flex">
                        ...
                        <x-button primary label="Save" wire:click="updateServiceTypeDetails({{ $selectedServiceTypeId }})" />
                    </div>
                </div>
            </x-slot>
        </x-card>
